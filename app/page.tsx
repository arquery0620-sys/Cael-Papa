...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex flex-col relative overflow-hidden">
          ...
          <div className="w-72 bg-white h-full flex flex-col shadow-xl">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-1">
              {[{ href: "/", label: "Home", icon: HomeIcon }, { href: "/chat", label: "Chat", icon: ChatBubbleLeftIcon }].map(({ href, label, icon: Icon }) => (
                <Link key={href} href={href} onClick={() => setShowMenu(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#faf8f5]">
                  <Icon className="w-5 h-5 text-[#c4b5a0]" />
                  <span className="font-[family-name:var(--font-cormorant)] text-lg text-[#2c2018]">{label}</span>
                </Link>
              ))}
              ...
              {openMore && (
                <div className="ml-8 flex flex-col gap-1">
                  {[{ href: "/diary", label: "Diary", icon: BookOpenIcon }, { href: "/board", label: "Board", icon: BookmarkIcon }, { href: "/usage", label: "Usage", icon: ChartBarIcon }].map(({ href, label, icon: Icon }) => (
                    <Link key={href} href={href} onClick={() => setShowMenu(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-[#faf8f5]">
              ...
              {openSettings && (
                <div className="ml-8 flex flex-col gap-1">
                  {[{ href: "/settings", label: "Persona", icon: PencilSquareIcon }, { href: "/settings", label: "API Key", icon: KeyIcon }].map(({ href, label, icon: Icon }) => (
                    <Link key={label} href={href} onClick={() => setShowMenu(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-[#faf8f5]">
                      <Icon className="w-3.5 h-3.5 text-[#c4b5a0]" />
                      <span className="font-[family-name:var(--font-cormorant)] text-sm text-[#2c2018]">{label}</span>
                    </Link>
                  ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#faf8f5] border-t border-[#f0ebe3] flex items-center justify-around px-6 py-4 pb-8 z-10">
